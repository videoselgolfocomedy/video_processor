...
import { Save, Plus, X, Copy, Trash2, ArrowRight } from 'lucide-react';
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (store.phase === 'timeline' && store.activeReelId) {
        ...
        if (e.key === 'Delete' || e.key === 'Backspace') {
          e.preventDefault();
          if (e.shiftKey && store.selectedClipId) {
        ...
        if (e.key === 'q' && !e.ctrlKey && !e.metaKey) {
          ...
          if (store.selectedClipId) {
            store.trimClip(rid, store.selectedClipId, 'in', store.currentTimeMs);
          }
        }
        ...
        if (e.key === 'w' && !e.ctrlKey && !e.metaKey) {
          e.preventDefault();
          if (store.selectedClipId) {
            store.trimClip(rid, store.selectedClipId, 'out', store.currentTimeMs);
          }
        }
    ...
    <div className="flex h-full flex-col">
      ...
      {!activeReelId ? (
        ...
        <div className="flex flex-1 min-h-0">
          ...
          {/* Right: preview + style + constraints */}
